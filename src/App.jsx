import { useLanguage, LanguageSwitcher } from "./i18n";import { useState, useEffect, useRef, useCallback } from "react";
...
function SectionHeader({ title, subtitle, isPrivate }) {const { tr, language } = useLanguage();
  return (
    <div style={{ marginBottom: 36 }}>
      <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 5, opacity: 0.25, marginBottom: 8, fontWeight: 500 }}>
        {tr(isPrivate ? "Private" : "Section")}
      </div>
      <h1 style={{ fontFamily: F.serif, fontWeight: 300, fontSize: 42, margin: 0, letterSpacing: -1, color: isPrivate ? C.orange : C.text, lineHeight: 1.1 }}>{tr(title)}</h1>
      {tr(subtitle && <p style={{ fontSize: 14, opacity: 0.35, marginTop: 10, lineHeight: 1.6, maxWidth: 600, fontWeight: 300 }}>{tr(subtitle)}</p>)}
      <div style={{ width: 40, height: 1, background: isPrivate ? C.orange : C.gold, opacity: 0.4, marginTop: 16 }} />
    </div>);

}

function NoteBox({ sectionKey }) {const { tr, language } = useLanguage();
  const k = "dn-hub-" + sectionKey;
  const [note, setNote] = useState("");
  const [saved, setSaved] = useState(false);
  useEffect(() => {(async () => {try {const r = { value: localStorage.getItem(k) };if (r?.value) setNote(r.value);} catch {}})();}, [k]);
  const save = async () => {try {localStorage.setItem(k, note);setSaved(true);setTimeout(() => setSaved(false), 1500);} catch {}};
  return (
    <div style={{ marginTop: 32, padding: 20, background: "rgba(var(--surface-rgb),0.02)", borderRadius: 12, border: "1px dashed rgba(var(--surface-rgb),0.06)" }}>
      <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 3, opacity: 0.25, marginBottom: 10 }}>{tr("Personal Notes")}</div>
      <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Write your thoughts...")}
...
function CardHover({ children, style, ...props }) {const { tr, language } = useLanguage();
  return (
...
// VIDEO CARD
// ============================================================
function VideoCard({ video, index }) {const { tr, language } = useLanguage();
      ...
      <div style={{ position: "relative", paddingTop: "56.25%", background: "#0a0a0a" }}>
        ...
        <div onClick={() => setLoaded(true)} style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", background: "linear-gradient(135deg, var(--sidebar-bg), var(--sidebar-bg))" }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: "rgba(200,169,81,0.15)", display: "flex", alignItems: "center", justifyContent: "center", border: "1px solid rgba(200,169,81,0.25)" }}>
              <span style={{ fontSize: 18, marginLeft: 2, color: C.gold }}>{tr("▶")}</span>
            </div>
          </div> :

        <iframe src={video.url + "?autoplay=1"} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: "none" }} allow="autoplay; encrypted-media" allowFullScreen title={tr(video.title)} />)
        }
      </div>
      <div style={{ padding: "14px 16px 16px" }}>
        <span style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, padding: "2px 8px", borderRadius: 4, background: (tc[video.tag] || "#888") + "18", color: tc[video.tag] || "#888", fontWeight: 600 }}>{tr(video.tag)}</span>
        <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 14, lineHeight: 1.4, opacity: 0.85, marginTop: 8 }}>{tr(video.title)}</div>
      </div>
    </div>);
...
// FIRE CALCULATOR
// ============================================================
function FIRECalculator() {const { tr, language } = useLanguage();
  ...
  <div style={{ marginBottom: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
        <span style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 2, opacity: 0.35 }}>{tr(label)}</span>
        <span style={{ fontSize: 14, fontFamily: F.serif, color: C.gold }}>{tr(prefix || "")}{tr(typeof val === "number" ? val.toLocaleString(language === "cs" ? "cs-CZ" : "en-US") : val)}</span>
      </div>
      <input aria-label={tr(label)} type="range" min={min} max={max} step={step || 1} value={val} onChange={(e) => set(Number(e.target.value))}
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
        ...
        <div style={{ background: C.card, borderRadius: 16, padding: 24, border: "1px solid " + C.border }}>
          <div style={{ fontFamily: F.serif, fontSize: 18, marginBottom: 20, fontWeight: 600 }}>{tr("Your Numbers")}</div>
          {tr(inp("Current Age", age, setAge, 18, 65))}
          {tr(inp("Annual Income", income, setIncome, 10000, 500000, 5000, "$"))}
          {tr(inp("Annual Expenses", expenses, setExpenses, 10000, 300000, 5000, "$"))}
          {tr(inp("Currently Saved", saved, setSaved, 0, 2000000, 10000, "$"))}
          {tr(inp("Expected Return %", returnRate, setReturnRate, 1, 15, 0.5))}
        </div>

        {/* Summary */}
        <div style={{ background: C.card, borderRadius: 16, padding: 24, border: "1px solid " + C.border }}>
          <div style={{ fontFamily: F.serif, fontSize: 18, marginBottom: 20, fontWeight: 600 }}>{tr("FIRE Dashboard")}</div>
          {tr([
          { label: "FI Number (25x expenses)", value: "$" + fiNumber.toLocaleString(language === "cs" ? "cs-CZ" : "en-US"), color: C.gold },
          { label: "Annual Savings", value: "$" + annualSavings.toLocaleString(language === "cs" ? "cs-CZ" : "en-US"), color: annualSavings > 0 ? "var(--success)" : "#e86f68" },
          ...
          <div key={i} style={{ padding: "14px 18px", background: "rgba(var(--surface-rgb),0.02)", borderRadius: 10, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontSize: 12, opacity: 0.4 }}>{tr(m.label)}</span>
              <span style={{ fontFamily: F.serif, fontSize: 18, fontWeight: 600, color: m.color }}>{tr(m.value)}</span>
            </div>
          ))}
          <div style={{ marginTop: 16, padding: "12px 16px", background: sr >= 50 ? "rgba(126,200,160,0.06)" : "rgba(200,169,81,0.06)", borderRadius: 10, fontSize: 12, opacity: 0.5, lineHeight: 1.6 }}>
            {tr(sr >= 50 ? "Excellent savings rate! You're on the fast track to FI." : sr >= 25 ? "Good progress. Increasing savings rate is the biggest lever." : "Focus on reducing expenses or increasing income to accelerate your timeline.")}
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 24, background: C.card, borderRadius: 16, padding: 24, border: "1px solid " + C.border, overflowX: "auto" }}>
        <div style={{ fontFamily: F.serif, fontSize: 18, marginBottom: 16, fontWeight: 600 }}>{tr("Growth Projection")}</div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
          {tr(projections.map((p, i) =>
          <div key={i} style={{ padding: "16px 14px", background: p.fiReached ? "rgba(126,200,160,0.06)" : "rgba(var(--surface-rgb),0.02)", borderRadius: 12, textAlign: "center", border: "1px solid " + (p.fiReached ? "rgba(126,200,160,0.15)" : C.border), animation: `fadeUp 0.4s ease ${i * 0.05}s both` }}>
              <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, opacity: 0.3, marginBottom: 6 }}>{tr(p.year)}{tr(" Years")}</div>
              <div style={{ fontFamily: F.serif, fontSize: 20, fontWeight: 600, color: p.fiReached ? "var(--success)" : C.text }}>{tr(" $")}
              {tr(fmt(p.value))}
              </div>
              <div style={{ fontSize: 10, opacity: 0.25, marginTop: 4 }}>{tr("Age ")}{tr(p.age)}</div>
              {tr(p.fiReached && <div style={{ fontSize: 9, color: "var(--success)", marginTop: 6, textTransform: "uppercase", letterSpacing: 1 }}>{tr("FI Reached")}</div>)}
            </div>
          ))}
        </div>
          ...
          {tr(projections.map((p, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                <div style={{ fontSize: 9, opacity: 0.3 }}>{tr("$")}{tr(fmt(p.value))}</div>
                ...
                <div style={{ fontSize: 9, opacity: 0.2 }}>{tr(p.year)}{language === "cs" ? " r." : "y"}</div>
              </div>);
...
// ============================================================
function InvestmentCalculator() {const { tr, language } = useLanguage();
  ...
  <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
        <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 2, opacity: 0.35 }}>{tr(label)}</span>
        <span style={{ fontSize: 13, fontFamily: F.serif, color: C.gold }}>{tr(suffix === "%" ? val + suffix : "$" + val.toLocaleString(language === "cs" ? "cs-CZ" : "en-US"))}</span>
      </div>
      <input aria-label={tr(label)} type="range" min={0} max={max} step={step || 1} value={val} onChange={(e) => set(Number(e.target.value))}
    ...
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 16 }}>
        ...
        <div style={{ background: C.card, borderRadius: 16, padding: 22, border: "1px solid " + C.border }}>
          <div style={{ fontFamily: F.serif, fontSize: 16, marginBottom: 16, fontWeight: 600 }}>{tr("Current Holdings")}</div>
          {tr(inp2("Stocks / ETFs", stocks, setStocks, 500000, 5000))}
          {tr(inp2("Crypto", crypto, setCrypto, 500000, 1000))}
          {tr(inp2("Bonds", bonds, setBonds, 200000, 1000))}
          {tr(inp2("Real Estate Equity", realEstate, setRealEstate, 1000000, 10000))}
          {tr(inp2("Total Debt", debt, setDebt, 500000, 5000))}
          {tr(inp2("Monthly Savings", monthlySavings, setMonthlySavings, 10000, 100))}
        </div>

        {/* Return rates */}
        <div style={{ background: C.card, borderRadius: 16, padding: 22, border: "1px solid " + C.border }}>
          <div style={{ fontFamily: F.serif, fontSize: 16, marginBottom: 16, fontWeight: 600 }}>{tr("Expected Returns")}</div>
          {tr(inp2("Stock Return", stockReturn, setStockReturn, 20, 0.5, "%"))}
          {tr(inp2("Crypto Return", cryptoReturn, setCryptoReturn, 30, 0.5, "%"))}
          {tr(inp2("Bond Return", bondReturn, setBondReturn, 10, 0.5, "%"))}
          {tr(inp2("Real Estate Return", reReturn, setReReturn, 15, 0.5, "%"))}
          {tr(inp2("Debt Interest Rate", debtRate, setDebtRate, 25, 0.5, "%"))}

          <div style={{ marginTop: 16, padding: "14px 16px", background: "rgba(200,169,81,0.06)", borderRadius: 10 }}>
            <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 2, opacity: 0.3, marginBottom: 6 }}>{tr("Net Worth Today")}</div>
          ...
          <div style={{ marginTop: 14, display: "flex", flexWrap: "wrap", gap: 8 }}>
            {tr([{ l: "Stocks", v: stocks, c: colors.stocks }, { l: "Crypto", v: crypto, c: colors.crypto }, { l: "Bonds", v: bonds, c: colors.bonds }, { l: "RE", v: realEstate, c: colors.realEstate }].filter((x) => x.v > 0).map((x, i) =>
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: x.c }} />
                <span style={{ fontSize: 10, opacity: 0.4 }}>{tr(x.l)} {tr(totalInv > 0 ? (x.v / totalInv * 100).toFixed(0) + "%" : "")}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ marginTop: 20, background: C.card, borderRadius: 16, padding: 24, border: "1px solid " + C.border, overflowX: "auto" }}>
        <div style={{ fontFamily: F.serif, fontSize: 16, marginBottom: 16, fontWeight: 600 }}>{tr("Portfolio Projection")}</div>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(var(--surface-rgb),0.06)" }}>
              {tr(["Years", "Stocks", "Crypto", "Bonds", "Real Estate", "Debt", "Net Total"].map((h, i) =>
              <th key={i} style={{ padding: "8px 6px", textAlign: i === 0 ? "left" : "right", fontSize: 9, textTransform: "uppercase", letterSpacing: 2, opacity: 0.3, fontWeight: 500 }}>{tr(h)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            ...
            <tr key={i} style={{ borderBottom: "1px solid rgba(var(--surface-rgb),0.03)", animation: `fadeUp 0.3s ease ${i * 0.05}s both` }}>
                <td style={{ padding: "10px 6px", fontFamily: F.serif, fontWeight: 600, opacity: 0.7 }}>{tr(r.year)}{language === "cs" ? " r." : "y"}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", color: colors.stocks }}>{tr(fmt(r.stocks))}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", color: colors.crypto }}>{tr(fmt(r.crypto))}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", color: colors.bonds }}>{tr(fmt(r.bonds))}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", color: colors.realEstate }}>{tr(fmt(r.realEstate))}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", color: colors.debt }}>{tr(r.debt > 0 ? "-" + fmt(r.debt) : "$0")}</td>
                <td style={{ padding: "10px 6px", textAlign: "right", fontFamily: F.serif, fontWeight: 700, fontSize: 14, color: r.total >= 0 ? "var(--success)" : "#e86f68" }}>{tr(fmt(r.total))}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          {tr(data.map((r, i) => {
            ...
            return (
              <div key={i} style={{ flex: 1, textAlign: "center" }}>
                <div style={{ fontSize: 9, opacity: 0.3, marginBottom: 4 }}>{tr(fmt(r.total))}</div>
                ...
                <div style={{ fontSize: 9, opacity: 0.2, marginTop: 4 }}>{tr(r.year)}{language === "cs" ? " r." : "y"}</div>
              </div>);
...
// ============================================================
function LongevityCard({ item, index }) {const { tr, language } = useLanguage();
  ...
  return (
    <div style={{ animation: `fadeUp 0.5s ease ${index * 0.06}s both` }}>
      ...
      onMouseLeave={(e) => {e.currentTarget.style.borderColor = C.border;}}>
        ...
        <span style={{ fontSize: 22, opacity: 0.2, marginTop: 2, flexShrink: 0 }}>{tr(item.icon)}</span>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 18 }}>{tr(item.title)}</div>
            <span style={{ fontSize: 10, opacity: 0.15, transform: open ? "rotate(180deg)" : "", transition: "transform 0.3s" }}>{tr("▼")}</span>
          </div>
          <div style={{ fontSize: 13, opacity: 0.5, lineHeight: 1.7, marginTop: 4 }}>{tr(item.desc)}</div>
        </div>
      </div>

      {tr(open &&
      <div style={{ padding: "0 0 0 62px", marginTop: 2 }}>
          {tr(item.details.map((d, i) =>
        <div key={i} style={{ background: "rgba(200,169,81,0.03)", borderRadius: 12, padding: "16px 20px", marginBottom: 6, borderLeft: "2px solid rgba(200,169,81,0.15)", animation: `fadeUp 0.3s ease ${i * 0.05}s both` }}>
              <div style={{ fontFamily: F.serif, fontSize: 14, fontWeight: 600, color: C.gold, marginBottom: 6 }}>{tr(d.heading)}</div>
              <div style={{ fontSize: 12, opacity: 0.5, lineHeight: 1.7 }}>{tr(d.text)}</div>
            </div>
        ))}
        </div>)
      }
...
// PHILOSOPHY CARD
// ============================================================
function PhiloCard({ card, index }) {const { tr, language } = useLanguage();
  const [open, setOpen] = useState(false);
  return (
    <div onClick={() => setOpen(!open)} style={{
    ...
    onMouseLeave={(e) => {e.currentTarget.style.borderColor = C.border;e.currentTarget.style.borderLeftColor = card.color + "44";}}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 20, color: card.color }}>{tr(card.title)}</div>
          <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 3, opacity: 0.2, marginTop: 3 }}>{tr(card.school)}</div>
        </div>
        <span style={{ opacity: 0.15, fontSize: 10, transform: open ? "rotate(180deg)" : "", transition: "transform 0.3s" }}>{tr("▼")}</span>
      </div>
      <div style={{ marginTop: 14, fontFamily: F.serif, fontSize: 15, opacity: 0.55, fontStyle: "italic", lineHeight: 1.6 }}>{tr(card.core)}</div>
      {tr(open &&
      <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(var(--surface-rgb),0.04)" }}>
          {tr(card.ideas.map((idea, i) =>
        <div key={i} style={{ fontSize: 13, opacity: 0.45, padding: "5px 0 5px 14px", borderLeft: `1px solid ${card.color}25`, marginBottom: 4 }}>{tr(idea)}</div>
        ))}
        </div>)
      }
    </div>);
...
// MAIN APP
// ============================================================
export default function App() {const { tr, language } = useLanguage();
  ...
  const renderSection = () => {
    switch (active) {
      ...
      <div className="home-topline"><span>{tr("DAVID / PERSONAL HUB")}</span><a href="https://david-roadmap.vercel.app/">{tr("Rozcestník ↗")}</a></div>
      <section className="hub-hero"><div className="hero-shade" /><div className="hub-hero-copy"><span>{tr("DAVIDŮV OSOBNÍ ZÁPISNÍK")}</span><h1>{tr("Místo pro trénink")}<br /><em>{tr("i další plány.")}</em></h1><p>{tr("Sbírám si tu videa a knihy spolu s poznámkami k ultimate frisbee, regeneraci a zdraví. Finanční kalkulačky mi pomáhají porovnat, jak se mění plán podle výdajů a úspor.")}</p><button onClick={() => setActive("motivation")}>{tr("Prohlédnout videa ↗")}</button></div></section>
      <div className="home-section-title"><h2>{tr("Čemu se chceš věnovat?")}</h2><span>{tr("Vyber si téma.")}</span></div>
      <div className="home-paths">{tr([
            ...
            map((item) => <button className="home-path" key={item.id} onClick={() => setActive(item.id)}><img src={"/images/" + item.image + ".jpg"} alt={tr("")} /><span><small>{tr(item.detail)}</small><strong>{tr(item.title)}</strong></span><b>{tr("↗")}</b></button>))}</div>
      <div className="home-utilities"><div><small>{tr("KNIHOVNA A FINANČNÍ PLÁN")}</small><h2>{tr("Knihy k přečtení")}<br />{tr("a plán úspor.")}</h2></div><button onClick={() => setActive("fire-calc")}>{tr("FIRE kalkulačka ↗")}</button><button onClick={() => setActive("books")}>{tr("Moje knihovna ↗")}</button></div>
    </div>;


      case "motivation":return (
          <div>
        <SectionHeader title={tr("Motivation")} subtitle={tr("Talks and interviews I return to. Filter by topic and choose something to watch.")} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {tr(filteredVideos.map((v, i) => <VideoCard key={v.id} video={v} index={i} />))}
        </div>
      ...
      case "mindset":return (
          <div>
        <SectionHeader title={tr("Mindset")} subtitle={tr("Quotes grouped by theme. Pick an idea, reflect on it, and add your own notes.")} />
        {/* Category tabs */}
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 28 }}>
          {tr(MINDSET_CATEGORIES.map((cat, i) =>
        ...
        <div style={{ columns: "340px 2", gap: 16 }}>
          {tr(MINDSET_CATEGORIES[mindsetCat].quotes.map((m, i) =>
              ...
              <div style={{ position: "absolute", top: 12, left: 18, fontSize: 44, opacity: 0.05, fontFamily: F.serif, fontWeight: 700 }}>{tr("“")}</div>
              <div style={{ fontFamily: F.serif, fontSize: 17, lineHeight: 1.7, fontStyle: "italic", opacity: 0.75, position: "relative", zIndex: 1 }}>{tr(m.quote)}</div>
              <div style={{ marginTop: 14, fontSize: 11, textTransform: "uppercase", letterSpacing: 3, opacity: 0.3 }}>{tr("—")} {tr(m.author)}</div>
            </div>
              ))}
        </div>
      ...
      case "philosophy":return (
          <div>
        <SectionHeader title={tr("Philosophy")} subtitle={tr("Ideas about decisions, uncertainty, and what matters. Open a card to read more.")} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
          {tr(PHILOSOPHY.map((c, i) => <PhiloCard key={i} card={c} index={i} />))}
        </div>
      ...
      case "training":return (
          <div>
        <SectionHeader title={tr("Training")} subtitle={tr("Notes on strength, conditioning, and recovery for ultimate frisbee.")} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 16 }}>
          {tr(TRAINING.map((s, i) =>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                <span style={{ color: C.gold, fontSize: 18 }}>{tr(s.icon)}</span>
                <span style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 20 }}>{tr(s.title)}</span>
              </div>
              <div style={{ fontSize: 11, opacity: 0.3, marginBottom: 14, textTransform: "uppercase", letterSpacing: 2 }}>{tr(s.desc)}</div>
              {tr(s.items.map((item, j) =>
                <div key={j} style={{ fontSize: 13, opacity: 0.55, padding: "7px 0 7px 14px", borderLeft: "1px solid rgba(200,169,81,0.15)", marginBottom: 4, lineHeight: 1.5 }}>{tr(item)}</div>
                ))}
            </div>
              ))}
        </div>
      ...
      case "longevity":return (
          <div>
        <SectionHeader title={tr("Longevity")} subtitle={tr("Notes on sleep, movement, nutrition, and healthy aging. Open a topic to explore it.")} />
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {tr(LONGEVITY_DATA.map((item, i) => <LongevityCard key={i} item={item} index={i} />))}
        </div>
      ...
      case "books":return (
          <div>
        <SectionHeader title={tr("Bookshelf")} subtitle={tr("My reading list, with a short takeaway from each book.")} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 14 }}>
          {tr(BOOK_SHELF.map((b, i) =>
              <CardHover key={i} style={{ padding: "20px 18px", animation: `fadeUp 0.4s ease ${i * 0.04}s both` }}>
              <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, opacity: 0.25, marginBottom: 10, color: C.gold }}>{tr(b.category)}</div>
              <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 16, lineHeight: 1.3, marginBottom: 6 }}>{tr(b.title)}</div>
              <div style={{ fontSize: 12, opacity: 0.35 }}>{tr(b.author)}</div>
            </CardHover>
              ))}
        </div>
      ...
      case "improvement":return (
          <div>
        <SectionHeader title={tr("Self-Improvement")} subtitle={tr("Practical approaches to habits, focus, and learning. Keep notes on what works for you.")} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {tr(SELF_IMPROVEMENT.map((s, i) =>
              ...
              <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 20, marginBottom: 14, color: s.color }}>{tr(s.title)}</div>
              {tr(s.items.map((p, j) =>
                <div key={j} style={{ fontSize: 13, opacity: 0.5, padding: "6px 0 6px 14px", borderLeft: `1px solid ${s.color}33`, marginBottom: 6, lineHeight: 1.5 }}>{tr(p)}</div>
                ))}
            </div>
              ))}
        </div>
      ...
      case "fire":return (
          <div>
        <SectionHeader title={tr("Financial Freedom")} subtitle={tr("The basics of saving, investing, and planning for financial independence.")} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
          {tr(FIRE_CONCEPTS.map((s, i) =>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                <span style={{ color: C.gold, fontSize: 16 }}>{tr(s.icon)}</span>
                <span style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 18 }}>{tr(s.title)}</span>
              </div>
              <div style={{ fontSize: 13, opacity: 0.5, lineHeight: 1.7 }}>{tr(s.content)}</div>
            </div>
              ))}
        </div>
      ...
      case "fire-calc":return (
          <div>
        <SectionHeader title={tr("FIRE Calculator")} subtitle={tr("Adjust income, spending, savings, and expected return to compare long-term scenarios. These are estimates, not guaranteed outcomes.")} />
        <FIRECalculator />
      ...
      case "portfolio":return (
          <div>
        <SectionHeader title={tr("Portfolio Projector")} subtitle={tr("Explore how contributions, asset allocation, and assumed returns affect a portfolio over time. Projections are illustrative.")} />
      ...
      case "my-life":return (
          <div>
        <SectionHeader title={tr("My Life")} subtitle={tr("My background, values, and priorities.")} isPrivate />
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {tr(MY_LIFE.map((item, i) =>
              ...
              <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: 3, color: C.orange, marginBottom: 8, fontWeight: 600 }}>{tr(item.label)}</div>
              <div style={{ fontSize: 14, lineHeight: 1.7, opacity: 0.7 }}>{tr(item.text)}</div>
            </div>
              ))}
        </div>
      ...
      case "my-career":return (
          <div>
        <SectionHeader title={tr("My Career")} subtitle={tr("Roles, experience, and what I want to work on next.")} isPrivate />
          ...
          {tr(MY_CAREER.map((item, i) =>
              ...
              <div style={{ background: "rgba(212,118,78,0.04)", borderRadius: 14, padding: "16px 22px", border: "1px solid rgba(212,118,78,0.08)" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <span style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 16, color: C.orange }}>{tr(item.phase)}</span>
                  <span style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 2, opacity: 0.25 }}>{tr(item.year)}</span>
                </div>
                <div style={{ fontSize: 13, opacity: 0.6, lineHeight: 1.6 }}>{tr(item.detail)}</div>
              </div>
            </div>
              ))}
        </div>
      ...
      case "my-achievements":return (
          <div>
        <SectionHeader title={tr("Achievements")} subtitle={tr("Milestones from work, sport, and projects.")} isPrivate />
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {tr(MY_ACHIEVEMENTS.map((g, i) =>
              ...
              <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 18, color: C.orange, marginBottom: 14 }}>{tr(g.area)}</div>
              {tr(g.items.map((item, j) =>
                <div key={j} style={{ fontSize: 13, opacity: 0.6, padding: "6px 0 6px 16px", borderLeft: "1px solid rgba(212,118,78,0.15)", marginBottom: 4, lineHeight: 1.5 }}>
                  <span style={{ color: C.orange, marginRight: 8, fontSize: 10 }}>{tr("★")}</span>{tr(item)}
                </div>
                ))}
            </div>
              ))}
      ...
      case "my-failures":return (
          <div>
        <SectionHeader title={tr("Failures")} subtitle={tr("Things that did not work, what I learned, and what I want to do differently.")} isPrivate />
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {tr(MY_FAILURES.map((f, i) =>
              <div key={i} style={{ background: "rgba(212,118,78,0.04)", borderRadius: 16, padding: 24, border: "1px solid rgba(212,118,78,0.08)", animation: `fadeUp 0.5s ease ${i * 0.08}s both` }}>
              <div style={{ fontFamily: F.serif, fontWeight: 600, fontSize: 18, marginBottom: 8 }}>{tr(f.lesson)}</div>
              <div style={{ fontSize: 13, opacity: 0.5, lineHeight: 1.7, marginBottom: 14 }}>{tr(f.detail)}</div>
              <div style={{ padding: "14px 18px", background: "rgba(212,118,78,0.06)", borderRadius: 10, fontSize: 13, lineHeight: 1.6, opacity: 0.65 }}>
                <span style={{ color: C.orange, fontWeight: 700, fontSize: 10, textTransform: "uppercase", letterSpacing: 2 }}>{tr("Reflection")}</span>
                <div style={{ marginTop: 6, fontStyle: "italic" }}>{tr(f.reflection)}</div>
              </div>
            </div>
              ))}
        </div>
  ...
  return (
    <div className="hub-shell" style={{ fontFamily: F.sans, background: C.bg, color: C.text, minHeight: "100vh", display: "flex", position: "relative" }}>
      ...
      <button aria-label={tr("Otevřít navigaci")} aria-expanded={sidebarOpen} onClick={() => setSidebarOpen(!sidebarOpen)} className="mob-btn" style={{ display: "none", position: "fixed", top: 14, left: 14, zIndex: 1001, background: "rgba(var(--surface-rgb),0.05)", border: "1px solid rgba(var(--surface-rgb),0.08)", borderRadius: 10, padding: "10px 14px", color: C.text, cursor: "pointer", fontSize: 16, backdropFilter: "blur(12px)" }}>{tr("☰")}</button>

      {tr(sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 998, backdropFilter: "blur(4px)" }} />)}

      <LanguageSwitcher /><button className="theme-toggle" aria-label={tr(theme === "dark" ? "Zapnout světlý vzhled" : "Zapnout tmavý vzhled")} onClick={() => setTheme((t) => t === "dark" ? "light" : "dark")}><span aria-hidden="true">{tr(theme === "dark" ? "☀" : "☾")}</span> {tr(theme === "dark" ? "Světlý vzhled" : "Tmavý vzhled")}</button>
      {/* Sidebar */}
      <div className={`sidebar ${sidebarOpen ? "open" : ""}`} style={{
        ...
        <div onClick={handleLogo} style={{ fontFamily: F.serif, fontWeight: 300, fontSize: 28, cursor: "default", userSelect: "none", padding: "0 10px", marginBottom: 2, letterSpacing: 4, color: secret ? C.orange : C.gold, transition: "color 0.6s ease" }}>{tr("DN")}</div>
        <div style={{ fontSize: 9, textTransform: "uppercase", letterSpacing: 4, opacity: 0.2, padding: "0 10px", marginBottom: 32 }}>
          {tr(secret ? "Private Mode" : "Personal Hub")}
        </div>

        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 8, textTransform: "uppercase", letterSpacing: 4, opacity: 0.15, padding: "0 10px", marginBottom: 10 }}>{tr("Explore")}</div>
          {tr([{ id: "home", label: "Přehled", icon: "◈" }, ...NAV_PUBLIC].map((s) =>
          ...
          onMouseLeave={(e) => {if (active !== s.id) e.currentTarget.style.color = "rgba(var(--ink-rgb),0.35)";}}>
              <span style={{ fontSize: 12, width: 18, textAlign: "center", opacity: 0.6 }}>{tr(s.icon)}</span>{tr(s.label)}
            </button>
          ))}

          {tr(secret &&
          <>
              <div style={{ fontSize: 8, textTransform: "uppercase", letterSpacing: 4, opacity: 0.15, padding: "0 10px", marginTop: 24, marginBottom: 10, color: C.orange }}>{tr("Private")}</div>
              {tr(NAV_PRIVATE.map((s) =>
            <button type="button" className="nav-item" aria-current={active === s.id ? "page" : undefined} key={s.id} onClick={() => {setActive(s.id);setSidebarOpen(false);window.scrollTo({ top: 0 });}} style={{
              padding: "8px 12px", borderRadius: 10, cursor: "pointer", fontSize: 13, fontWeight: 400,
              marginBottom: 1, display: "flex", alignItems: "center", gap: 10, transition: "all 0.25s",
        ...
        <div style={{ padding: "12px 10px", fontSize: 9, opacity: 0.12, letterSpacing: 2, textTransform: "uppercase" }}>{tr("©")}{tr(" DN ")}{tr(new Date().getFullYear())}</div>
      </div>
      ...
      {tr(secret && <div style={{ position: "fixed", bottom: 20, right: 20, width: 6, height: 6, borderRadius: "50%", background: C.orange, animation: "pulse 2.5s ease-in-out infinite" }} />)}
